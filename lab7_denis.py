...
def remove_duplicates_set(s):
    ...
    for i in range(len(s)):
        if s[i] not in set1:
            set1.add(s[i])
            result += s[i] # add to the resulting string
    return result
    

# Your unit tests
assert remove_duplicates_set("aaa") == "a"
assert remove_duplicates_set("duplicates") == "duplicates"
assert remove_duplicates_set("set1_comp115_") == "set1_comp5"
assert remove_duplicates_set("money == $$$") == "money =$"
...
# Your unit tests
assert gem_counting("lalala", "ql") == 3
assert gem_counting("apple", "pp") == 2
assert gem_counting("gems1000", "gs0") == 5
assert gem_counting("abcdefghijklm", "nopqrstuvwxyz") == 0
...
def students_id(ids):
    ...
    for i in range(len(ids)):
        if ids[i] not in set1:
            set1.add(ids[i])
    return len(set1)
    

# Your unit tests
assert students_id(["001", "002", "003", "004"]) == 4
assert students_id(["099", "543", "052", "052", "051", "543"]) == 4
assert students_id(["000", "000", "000", "000", "080"]) == 2
...
# Your unit tests
assert students_id_occurrences(['000', '000', '000', '050', '022']) == {'000': 3, '050': 1, '022': 1}
assert students_id_occurrences(['002', '003', '001', '004', '012']) == {'002': 1, '003': 1, '001': 1, '004': 1, '012': 1}
assert students_id_occurrences(['100', '100', '200', '400', '100']) == {'100': 3, '200': 1, '400': 1}
...
def word_frequency(paragraph):
    paragraph = paragraph.split() # split to words
    id_dict = {}
    for id in paragraph:
        if id in id_dict:
            id_dict[id] += 1
...
# Your unit tests
assert word_frequency("I am stupid") == {'I': 1, 'am': 1, 'stupid': 1}
assert word_frequency("This is many words") == {'This': 1, 'is': 1, 'many': 1, 'words': 1}
assert word_frequency("python python python") == {'python': 3}
assert word_frequency("split the sentences to a list of words") == {'split': 1, 'the': 1, 'sentences': 1, 'to': 1, 'a': 1, 'list': 1, 'of': 1, 'words': 1}
