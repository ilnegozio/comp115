...
def convert_american_dollars(american_dollars):
    canadian_dollars = round(american_dollars * 1.34, 2)
    print(canadian_dollars)

convert_american_dollars(100.05)
...
def alarm_time(current_time, waiting_time):
    hours = current_time + waiting_time
    final_time = hours % 24
    days_passed = waiting_time // 24 # also, counts the amount of days passed
    print(f"Time on the clock will be {final_time} hours, with a total of {days_passed} days passed.")

alarm_time(current_time, waiting_time)
...
def find_max(nums):
    ...
    for i in range(len(list2)):
        if list2[i] > max_value:
            max_value = list2[i] # update max value
...
def squares(nums):
    ...
    for i in range(len(list5)):
        power_list.append((list5[i] ** 2)) # add a squared value to a new list
    print(power_list)
